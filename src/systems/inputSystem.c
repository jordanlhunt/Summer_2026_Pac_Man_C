...
// Turning for PAC-MAN is only allowed when centered on a given tile as described in dossier CH2_Cornering
static bool IsCenteredOnTile(Position *entityPosition)
{
  ...
  if (fabsf(entityPosition->offsetX) < TURN_TOLERANCE && fabsf(entityPosition->offsetY) < TURN_TOLERANCE)
  {
    isCentered = true;
  }
  return isCentered;
...
{
  if (!ECS_HasComponent(gameContext->playerEntity, COMPONENT_PLAYER_CONTROLLED))
  {
    printf("[inputSystem.c] - player does not have player_controlled component\n");
    return;
  }
